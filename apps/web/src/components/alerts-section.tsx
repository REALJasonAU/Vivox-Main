    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
      <div className="flex items-center justify-between">
        <h3 className="flex items-center gap-2 text-sm font-medium text-zinc-100">
          <Bell className="size-4 text-zinc-500" /> Resource Alerts
        </h3>
      ...
      <AnimatePresence initial={false}>
        {ruleList.map((rule) => (
          <motion.div
            ...
            className="mt-2 flex items-center gap-3 overflow-hidden rounded-lg border border-zinc-800 px-3 py-2 text-sm"
          >
            <span className={cn("size-2 rounded-full", rule.enabled ? "bg-vivox-500" : "bg-zinc-600")} />
      ...
      {ruleList.length === 0 && !creating && (
        <p className="mt-3 text-sm text-zinc-500">No alert rules configured.</p>
      )}
      ...
      <AnimatePresence>
        {creating && (
          <motion.div
            ...
            animate={{ opacity: 1, height: "auto" }}
            exit={{ opacity: 0, height: 0 }}
            className="mt-3 flex flex-wrap items-end gap-2 overflow-hidden rounded-lg border border-zinc-700 bg-zinc-950/50 p-3"
          >
            <select
              value={metric}
            ...
            <input
              ...
              className="h-9 w-24 rounded-lg border border-zinc-800 bg-zinc-950/50 px-3 font-mono text-sm text-zinc-100"
            />
            <span className="text-xs text-zinc-500">{metric === "cpu" ? "%" : "MB"}</span>
