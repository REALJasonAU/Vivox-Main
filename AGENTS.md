...
- User-facing UI copy uses **"servers"** not "services" (internal types/API may still say `service`).
- File manager defaults to **List mode** with a full-screen Monaco editor; in-browser Tree/VS Code modes are secondary toggles.
...
- **UI navigation:** `USER_NAV` — My Servers (`/dashboard`, `/services/*`). `ADMIN_NAV` — Dashboard, Servers (`/admin/servers`, edit at `/admin/servers/edit/[id]`), Users (`/admin/users`), Nodes, Audit, Templates (`/deploy`). Panel switcher (shield icon / profile menu). Deploy wizard and admin routes are **admin-only**.
- **Deploy wizard:** target **node** dropdown (regions removed); per-port bind IP + optional alias; numeric memory / **CPU threads** (1024 Docker shares = 1 thread; 100% = 1 core) / disk GB; owner user picker for admins. Ports validated against node conflicts.
- **Services & install:** Templates in `templates/*.yaml` with `install_script`; agent mounts `vivox-data-{serviceId}` → `/mnt/server`, runs install once (`.vivox-installed`), then `startup_cmd`. New template env vars **merge onto existing servers** with defaults (no overwrite). Backups default to `/var/lib/vivox/backups`. **Reinstall:** `POST /api/services/:id/reinstall` + UI in `service-controls.tsx`. **Force stop:** `POST /api/services/:id/force-stop` (SIGKILL); UI **Kill** after prolonged STOPPING.
- **Nodes & agents:** Register with **`name` only** — Ubuntu agents auto-detect CPU/RAM/disk on heartbeat (refresh every **6h** and on restart). Clone at `/opt/vivox-agent`; env `/etc/vivox-agent/agent.env`; **auto-updater** systemd timer (~5 min) runs `update-node.sh`. Edge scripts: `infra/scripts/install-node.sh`, `update-node.sh`, `node-agent-lib.sh` (update accepts `--token`, `--panel-url`, `--node-id`, `--control-addr`). Co-located panel+node: `NEXUS_CONTROL_ADDR=127.0.0.1:9090` (Pangolin proxies HTTPS only, not gRPC **9090**).
...
- **Production deploy:** Panel at `/opt/vivox` via `infra/scripts/install.sh`; updates via `infra/scripts/update.sh` (`--rebuild` when git is already up-to-date). Stack: `infra/prod/docker-compose.yml` + idempotent `infra/prod/setup.sh`. Pangolin on a **separate** host terminates SSL → port 3000; **9090** must reach the API for agents. Web Docker build: `ARG API_URL` before `npm run build`; requires committed `apps/web/package-lock.json`.
